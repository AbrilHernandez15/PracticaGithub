...
<div class="container mt-5 col-md-6">
    ...
    <div>
        ...
        <form method="post" action="{{ route('insecto.update', $insecto[0]->id) }}">
            @csrf 
            @method('PUT')
            <div>
                <label class="form-label" name="labelTitulo">Insecto:</label>
                <input type="text" class="form-control" name="txtInsecto" value="{{old ('txtInsecto', $insecto[0]->insecto)}}">
                <p class="fw-bold text-danger">{{$errors->first('txtInsecto')}}</p>
            </div>

            <div>
                <label class="form-label" name="labelRecuerdo">Tipo:</label>
                <input type="text" class="form-control" name="txtTipo" value="{{old ('txtTipo', $insecto[0]->tipo)}}">
